...
def build_pdf():
    ...
    # Title & Header
    story.append(Paragraph("WhatsApp Support Gateway - Concise Video Script", title_style))
    story.append(Paragraph("Teleprompter cue cards formatted for easy reading on your phone while recording on laptop.", sub_style))

    # Pre-Recording Checklist Box
    checklist_data = [
        [
            Paragraph("<b>RECORDING SETUP:</b><br/>"
                      "• Run: <code>uv run uvicorn gateway.main:app --host 0.0.0.0 --port 8000 --reload</code><br/>"
                      "• Open: <code>http://localhost:8000/demo</code> (100% zoom, both panes visible)<br/>"
                      "• Screen Record: Press <b>Print Screen</b> key (or use Loom / OBS)", checklist_style)
        ]
    ]
    ...
    for item in steps:
        card_content = [
            ...
            [Paragraph("<b>🎙️ SPOKEN SCRIPT (SAY THIS):</b>", label_voice)],
    ...
    story.append(Paragraph("Master Quick-Reference Cheat Sheet (All-In-One Table)", title_style))
    story.append(Spacer(1, 8))

    summary_table_data = [
        [
            Paragraph("What to Click / Type", th_style),
            Paragraph("What Happens on Screen", th_style),
            Paragraph("What to Say in Video", th_style)
        ]
    ]
    ...
    for item in steps:
        summary_table_data.append([
            ...
            Paragraph(item["visual"], td_visual),
            Paragraph(item["voice"], td_voice)
        ])

    st = Table(summary_table_data, colWidths=[120, 160, 280])
    ...
    doc.build(story)
    print(f"Generated concise PDF at: {PDF_PATH}")
